...
class TestEncodeCommand(unittest.TestCase):
    ...
    def test_encode_requires_secret(self):
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.cli, ["encode", "in.csv"])
            assert result.exit_code != 0
    ...
    def test_encode_febrl_data(self):
        ...
        with runner.isolated_filesystem():
            result = runner.invoke(
                cli.cli, ["encode", a_pii, "a", schema_file, "out.json"]
            )

            result_2 = runner.invoke(
                cli.cli, ["encode", a_pii, "a", schema_file, "out-2.json"]
            )
    ...
    def test_encode_wrong_schema(self):
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(
                cli.cli,
                ["encode", "--quiet", "--schema", schema_file, a_pii, "a", "b", "-"],
            )
    ...
    def test_encode_schemaerror(self):
        ...
        with runner.isolated_filesystem():
            result = runner.invoke(
                cli.cli, ["encode", a_pii, "horse", schema_file, "out.json"]
            )
        assert result.exit_code != 0
        assert "schema is not valid" in result.output
...
class TestFindSimilarityCommand(unittest.TestCase):
    ...
    def test_find_similarities_nonblocking(self):
        ...
        with temporary_file() as output_filename:
            ...
            self.assertEqual(result.exit_code, 0, msg=result.output)
            self.assertEqual(result.output.rstrip(), "Found 4962 matches")
